// One modification by macko in FormatScientific(): use 3 digits of exponent only if necessary (e+123), otherwise use two if necessary (e+45), otherwise use one (e+6).
// This is consistent with java and javascript, and partially with python (which never uses one digit, only two or three).
// To always print 3 digits in exponent (zero-padding if necessary), uncomment:
// #define PRINTFLOAT_DRAGON4_ALWAYS_3_DIGIT_EXPONENT
...
)
{
    ...
    if (bufferSize > 1)
    {
        tC8 exponentBuffer[5]; //we will need 3, 4 or 5 chars
        exponentBuffer[0] = 'e';
        if (printExponent >= 0)
        {
            exponentBuffer[1] = '+';
		...
		// modified by macko: use 3 digits of exponent only if necessary (e+123), otherwise use two if necessary (e+45), otherwise use one (e+6)
		unsigned int bufferIndex = 2;
#ifndef PRINTFLOAT_DRAGON4_ALWAYS_3_DIGIT_EXPONENT
		if (hundredsPlace != 0) //3 digits needed
#endif
			exponentBuffer[bufferIndex++] = (tC8)('0' + hundredsPlace);
#ifndef PRINTFLOAT_DRAGON4_ALWAYS_3_DIGIT_EXPONENT
		if (hundredsPlace != 0 || tensPlace != 0) //2 digits needed
#endif
			exponentBuffer[bufferIndex++] = (tC8)('0' + tensPlace);
		exponentBuffer[bufferIndex++] = (tC8)('0' + onesPlace);
		// now bufferIndex indicates how many characters of exponentBuffer were used

		// copy the exponent buffer into the output
		tU32 maxExponentSize = bufferSize - 1;
		tU32 exponentSize = (bufferIndex < maxExponentSize) ? bufferIndex : maxExponentSize;
